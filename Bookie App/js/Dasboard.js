...
// ── Hamburger / sidebar / overlay refs ───────────────────────
...
function openSidebar() {
  ...
  // Hide hamburger so it doesn't sit on top of sidebar links and block taps
...
function closeSidebar() {
  ...
  // Restore hamburger (only on mobile widths)
...
async function loadUserApplications() {
  ...
  try {
    ...
    try {
      ...
      container.innerHTML =
        '<div class="booking-card error"><h3>Error</h3><p>Server returned invalid data. Is the backend running?</p></div>';
      return;
    }
    ...
    if (!apps.length) {
      container.innerHTML =
        '<div class="booking-card empty"><h3>No submitted applications</h3>' +
        '<p>You have not applied for any hostel yet.<br>' +
        'Go to the <a href="Rooms.html">Rooms page</a> to book one.</p></div>';
      return;
    }
 
    apps.forEach(function(app) {
      var status      = app.applicationStatus || "Pending";
      ...
      card.innerHTML  =
        '<div style="flex:1;">' +
          ...
          '<div class="booking-body">' +
            ...
            '<p><strong>Room:</strong> ' + (app.roomId && app.roomId.roomNumber ? app.roomId.roomNumber : "Not Assigned") + '</p>' +
    ...
    console.error(err);
    container.innerHTML =
      '<div class="booking-card error"><h3>Error</h3><p>' + err.message + '</p></div>';
  }
}
